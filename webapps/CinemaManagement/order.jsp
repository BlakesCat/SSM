...
<body>
    ...
    <div class="tools">
    	...
    	<ul class="toolbar">
        <li class="click"><a href="orderAdd.jsp"><span><img src="images/t01.png" /></span>添加</a></li>
    ...
    <table class="tablelist">
        ...
        <th>订单序号<i class="sort"><img src="images/px.gif" /></i></th>
        <th>影厅编号</th>
        <th>电影名</th>
        <th>开场时间</th>
        <th>预定时间</th>
        ...
        String sql1="select count(*) as num from tbl_order";
        ...
        <td><%=list.get(i).get("OrderID")%></td>
        <td><%=list.get(i).get("HallID")%></td> 
        <td><%=list.get(i).get("MovieName")%></td> 
        <td><%=list.get(i).get("StartTime")%></td>
        <td><%=list.get(i).get("BuyDateTime")%></td>
        <td><%=list.get(i).get("CustomerPhone")%></td>
        <td><%=list.get(i).get("BankNum")%></td>
        <td><%=list.get(i).get("Price")%></td>
        <td><a href="orderUpdate.jsp?OrderID=<%=list.get(i).get("OrderID") %>" class="tablelink">更改</a>     <a href="orderDelete.jsp?em_number=<%=list.get(i).get("OrderID") %>"class="tablelink"> 删除</a></td>
        </tr> 
        <%} %>  
        </tbody>
    </table>
